...
class DiffusionModule(Module):
    ...
    def forward(
        ...
        if self.activation_checkpointing and self.training:
            ...
            s_trunk_expanded = (
                s_trunk if s_trunk.shape[0] >= multiplicity
                else s_trunk.repeat_interleave(multiplicity, 0)
            )
...
class AtomDiffusion(Module):
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            if (guidance_hooks is not None
                    and hasattr(guidance_hooks, 'pre_denoise_embedding')):
                network_condition_kwargs = guidance_hooks.pre_denoise_embedding(
                    atom_coords_noisy, t_hat, step_idx, num_sampling_steps,
                    structure_module=self,
            ...
            with torch.no_grad():
                ...
                for sample_ids_chunk in sample_ids_chunks:
                    chunk_kwargs = dict(network_condition_kwargs)
                    if (chunk_kwargs.get("s_trunk") is not None
                            and chunk_kwargs["s_trunk"].shape[0] > 1):
                        chunk_kwargs["s_trunk"] = chunk_kwargs["s_trunk"][
                            sample_ids_chunk
                        ]

                    dc = chunk_kwargs.get("diffusion_conditioning")
                    if dc is not None and dc["q"].shape[0] > 1:
                        chunk_kwargs["diffusion_conditioning"] = {
                            k: (
                                v[sample_ids_chunk]
    ...
    def sample_twisted(
        ...
        **network_condition_kwargs,
        ...
        """Sample with twisted diffusion (ConforMix faithful port).

        Uses the Feynman-Kac SMC particle filter with TwistedDDPMv2,
        mirroring the official ConforMix implementation exactly.

        Args:
            atom_mask: Atom mask tensor.
            num_sampling_steps: Number of diffusion steps (default: 200).
            multiplicity: Number of particles.
            max_parallel_samples: Max particles per batch for GPU memory.
            twist_fn: Twist function (log potential + gradient).
            ess_threshold: ESS resampling threshold (default: 1/3).
            **network_condition_kwargs: Passed to preconditioned_network_forward.

        Returns:
            Dict with sample_atom_coords, diff_token_repr, and trace data.
        """
        from functools import partial

        from shared.guidance.feynman_kac import smc_FK
        from shared.guidance.twisted_ddpm_v2 import TwistedDDPMv2

        num_sampling_steps = default(num_sampling_steps, self.num_sampling_steps)
        atom_mask = atom_mask.repeat_interleave(multiplicity, 0)
        batch_p = max_parallel_samples or min(25, multiplicity)

        # Sigma schedule
        sigmas = self.sample_schedule(num_sampling_steps)
        # ConforMix: constant gamma for all steps (never set to 0)
        gammas = torch.ones_like(sigmas) * self.gamma_0

        # Wrap Boltz-2 denoiser for TwistedDDPMv2
        def _denoise(atom_pos, t_hat, multiplicity):
        ...
        # Alignment function (Boltz-2 standard)
        alignment_fn = weighted_rigid_align if self.alignment_reverse_diff else None

        sampler = TwistedDDPMv2(
            sigmas=sigmas,
            gammas=gammas,
            atom_mask=atom_mask,
            ...
            device=self.device,
            step_scale=1.0,  # ConforMix paper: 1.0 (NOT Boltz-2 default)
            alignment_reverse_diff=self.alignment_reverse_diff,
            alignment_fn=alignment_fn,
        )

        (
            final_sample, log_w, normalized_w, resample_trace, ess_trace,
            log_w_trace, xt_trace, logp_trace, grad_trace,
        ) = smc_FK(
            ...
            verbose=True,
            log_xt_trace=True,
            extra_vals=dict(batch_p=batch_p),
        )

        return dict(
            sample_atom_coords=final_sample,
            diff_token_repr=None,
            ess_trace=ess_trace,
            log_w_trace=log_w_trace,
            xt_trace=xt_trace,
            logp_y_given_x0_trace=logp_trace,
            grad_log_potential_xt_trace=grad_trace,
        )
